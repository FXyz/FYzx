...
public class ClothMesh extends MeshView {
    ...
    private final DoubleProperty clothWidth = new SimpleDoubleProperty() {
        ...
        protected void invalidated() {
            //buildMesh(getDivisionsX(), getDivisionsY(), getClothWidth(), getClothHeight(), getStiffness());
        }
    };
    ...
    private class ClothTimer extends ScheduledService<Void> {
        ...
        protected void succeeded() {
            super.succeeded();
            mesh.updateUI();
            /*if(getTimeAsSeconds() >= windDelay){
             readyForWind = true;
             windDelay = getTimeAsSeconds() + (Math.PI * Math.random());
             log.log(Level.INFO, "\n{0}", getTimeAsSeconds() / windDelay);
             }*/
        }

...
public class RK4Integrator implements Integrator {
     ...
     * @return 
     */
    private Point3D acceleration(PhysicalState state, double dt) {        
        return new Point3D(
                state.getVelocity().subtract(state.getPrevVelocity()).getX() * dt,
                state.getVelocity().subtract(state.getPrevVelocity()).getY() * dt,
                state.getVelocity().subtract(state.getPrevVelocity()).getZ() * dt
        );
